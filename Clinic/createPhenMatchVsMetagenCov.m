...
isValid = is_valid(all_res, isMeasured);

all_res = all_res(isValid,:);
resistancemaps = get_resistance_matrix(all_res);

%% Which patients have both a UTI isolate and an FIT isolate in the data?

patients_w_fobt = unique(all_res.FOBT_ID(all_res.UTI_ID==0));
...
for p = 1:npatients
    personalFOBT_inds{p} = find(patients(p)==all_res.FOBT_ID & all_res.UTI_ID==0);
